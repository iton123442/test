...
class AWSHelper{
	...
	public static function findGameDetails($type, $provider_id, $game_code)
	{
		...
		$query = DB::Select("SELECT game_id,game_code,game_name,sub_provider_name FROM games inner join sub_providers sp using (sub_provider_id) WHERE game_code = '" . $game_code . "' AND sp.provider_id = '" . $provider_id . "' order by sp.sub_provider_id desc");
		$result = count($query);
