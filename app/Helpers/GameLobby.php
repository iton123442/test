...
class GameLobby{
    ...
    public static function nagaLaunchUrl($request){
        ...
        // Helper::savePLayerGameRound($game_code,$token,$provider);
        $registerurl = 'https://stg-bonanza.azureedge.net/?playerToken='.$request['token'].'&groupCode='.$groupCode.'&brandCode='.$brandCode. "&gameCode=" . $game_code . "&redirectUrl=".$exit_url;
        $client = new Client([
            'headers' => [
                'Content-Type' => 'application/json' 
            ],
        ]);
        $Register = $client->get($registerurl);
