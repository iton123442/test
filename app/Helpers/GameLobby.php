...
class GameLobby{
    ...
    public static function NoLimitLaunchUrl($data,$device){
        try {
        $url = config("providerlinks.nolimit.api_url").'device='.$device.'&hideExitButton=true'.'&language='.$data['lang'].'&operator='.config("providerlinks.nolimit.operator").'&game='.$data['game_code'].'&token='.$data['token'];
        return $url;
