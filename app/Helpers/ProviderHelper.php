...
class ProviderHelper{
	...
	public static function saveLog($method, $provider_id = 0, $request_data, $response_data)
	{
		...
		// if(env('SAVELOG')){
		// 	if(env('Al_DEBUG')){
		// 		return DB::connection(config('double_db.DB1'))->table('debug')->insert($data);
		// 	}else{
		// 		return DB::connection(config('double_db.DB1'))->table('seamless_request_logs')->insert($data);
		// 	}
		// }
	 ...
	 * 
	 */
	public static function mandatorySaveLog($method, $provider_id = 0, $request_data, $response_data)
	{
		try{
			// if(env('MANDATORY_LOG')){
				$data = [
					"method_name" => $method,
					"provider_id" => $provider_id,
					"request_data" => json_encode(json_decode($request_data)),
	...
	{
		try{
			if(env('SAVELOG_EXCEPTION', null) == $provider_id){
				$data = [
					"method_name" => $method,
					"provider_id" => $provider_id,
					"request_data" => json_encode(json_decode($request_data)),
	...
	public static function saveCLientErrorLog($method, $provider_id = 0, $request_data, $response_data)
	{
		try{
			if(env('SAVELOG_CLIENT_ERROR', null) == $provider_id){
				$data = [
					"method_name" => $method,
					"provider_id" => $provider_id,
					"request_data" => json_encode(json_decode($request_data)),
    ...
    public static function getClientDetails($type = "", $value = "", $gg=1, $providerfilter='all', $client_id = 1) {
		...
		# New Query
		$query = DB::select('select `p`.`client_id`,`c`.`country_code`, `p`.`player_id`, `p`.`email`, `p`.`client_player_id`,`p`.`language`,`p`.`balance` as `tw_balance`, `p`.`currency`, `p`.`test_player`, `p`.`username`,`p`.`created_at`,`pst`.`token_id`,`pst`.`player_token`,`pst`.`player_ip_address`,`pst`.`balance`,`c`.`client_url`,`c`.`default_currency`,`c`.`wallet_type`,`pst`.`status_id`,`p`.`display_name`,`op`.`client_api_key`,`op`.`client_code`,`op`.`client_access_token`,`op`.`operator_id`,`ce`.`player_details_url`,`ce`.`fund_transfer_url`,`ce`.`transaction_checker_url`,`p`.`created_at`, `c`.`connection_name`, `c`.`default_language`,  TIME_TO_SEC(TIMEDIFF( NOW(), `p`.`created_at`))/60 as `time` 
		from (select token_id, player_id, player_token, balance, player_ip_address,status_id from player_session_tokens pst '.$where.' '.$filter.') pst 
		inner join players as p using(player_id) 
		inner join clients as c using (client_id)
		inner join client_endpoints as ce using (client_id) 
	...
	public static function findGameDetails($type, $provider_id, $game_code)
	{
		...
		$query = DB::connection('default-read')->select("SELECT game_id,game_code,game_name,sub_provider_name as provider_name FROM games inner join sub_providers sp using (sub_provider_id) WHERE game_code = '" . $game_code . "' AND sp.provider_id = '" . $provider_id . "' order by sp.sub_provider_id desc");
		$result = count($query);
		return $result > 0 ? $query[0] : null;
	}
//Naga Games
	public static function findGameLaunchURL($type, $provider_id, $game_code)
	{
		// $query = DB::Select("SELECT game_id,game_code,game_name FROM games WHERE game_code = '" . $game_code . "' AND provider_id = '" . $provider_id . "'");
		// $query = DB::Select("SELECT game_id,game_code,game_name,provider_name FROM games inner join providers using (provider_id) WHERE game_code = '" . $game_code . "' AND provider_id = '" . $provider_id . "'");
	...
	public static function getGeneralDetailsV2($type, $roundId)
	{
		// $query = DB::Select("SELECT game_id,game_code,game_name FROM games WHERE game_code = '" . $game_code . "' AND provider_id = '" . $provider_id . "'");
		// $query = DB::Select("SELECT game_id,game_code,game_name,provider_name FROM games inner join providers using (provider_id) WHERE game_code = '" . $game_code . "' AND provider_id = '" . $provider_id . "'");
		$query = DB::Select("SELECT general_details as general_details FROM game_transaction_logs  WHERE round_id = '" . $roundId . "' AND game_transaction_type = '" . $type . "' order by game_trans_ext_id desc limit 1");
		$result = count($query);
		return $result > 0 ? $query[0] : null;
	}
	/**
	...
	public static function saveBalance($token){
		...
		try{	
			...
			// return $e->getMessage();
			ProviderHelper::saveLogWithExeption('PLAYER DETAILS LOG', 999, json_encode($datatosend), $e->getMessage() );
			return false;
		 }
	}
