...
class TransferWalletHelper{
	...
	public static function getInfoPlayerGameRound($player_token){
		...
		// select pgr.round_id, pst.player_token, g.game_name, g.game_code, g.game_id,ply.player_i
		// $query = DB::select('select g.game_name, g.game_code, g.game_id from player_game_rounds as pgr 
		// 					inner join player_session_tokens as pst using (player_token)
		// 					inner join games as g using (game_id)
		// 					inner join players as ply using (player_id)
		// 					where pgr.player_token = "'.$player_token.'"')	;
	...
	public static function getClientDetails($type = "", $value = "", $providerfilter = 'all')
	{
		...
		$query = DB::select('select `p`.`client_id`, `p`.`player_id`, `p`.`email`, `p`.`client_player_id`,`p`.`language`,`p`.`balance` as `tw_balance`, `p`.`currency`, `p`.`test_player`, `p`.`username`,`p`.`created_at`,`pst`.`token_id`,`pst`.`player_token`,`pst`.`balance`,`c`.`client_url`,`c`.`default_currency`,`pst`.`status_id`,`p`.`display_name`,`op`.`client_api_key`,`op`.`client_code`,`op`.`client_access_token`,`op`.`operator_id`,`ce`.`player_details_url`,`ce`.`fund_transfer_url`,`ce`.`transaction_checker_url`,`p`.`created_at` from player_session_tokens pst inner join players as p using(player_id) inner join clients as c using (client_id) inner join client_endpoints as ce using (client_id) inner join operator as op using (operator_id) '.$where.' '.$filter.'');
