...
class FiveMenController extends Controller
{
	...
	public function gameBet($request){
		...
		if ($client_details->balance < 0 ) {
			...
			Helper::saveLog('5men BET Not enough money ', $this->provider_db_id, json_encode($request), $response);
			return $response;
		}
		...
		try {
			...
			$existing_bet = TGGHelper::findGameTransactionExstingBet($bet_id, 'round_id');
			if ($existing_bet != 'false') {
				// $total_bet_amount = $existing_bet->bet_amount + $bet_amount;
				// $income = $total_bet_amount - $existing_bet->pay_amount;
				// TGGHelper::updateBetTransaction($existing_bet->game_trans_id, $existing_bet->pay_amount, $income, 5, $existing_bet->entry_id, $total_bet_amount);
				// $game_trans_id = $existing_bet->game_trans_id;

				$game_trans_id  = ProviderHelper::createGameTransaction($token_id, $game_code, $bet_amount,  $pay_amount, $method, $win_or_lost, null, $payout_reason, $income, $provider_trans_id, $request['data']['action_id']);
			} else {
	...
	public  function gameWin($request){
		...
		try{
			...
			Helper::saveLog('5men win duplicate', $this->provider_db_id, json_encode($request), $response);
			return $response;
		}
		...
		if (isset($string_to_obj->game->action)) {
			...
			switch ($string_to_obj->game->action) {
				...
				default:
					...
					$existing_bet = TGGHelper::findGameTransactionExstingBet($reference_transaction_uuid, 'round_id');
					$game_details = TGGHelper::findGameDetails('game_code', $this->provider_db_id, $game_id)[0];
					if ($existing_bet != 'false') {
						$amount = $request['data']['amount'];
						$transaction_uuid = $request['callback_id'];
		...
		if($game_refund == 'false'): // NO REFUND WAS FOUND PROCESS IT!
			...
			if($existing_transaction != 'false'): // IF BET WAS FOUND PROCESS IT!
				...
				if ($existing_transaction->win == 2) {
					...
					Helper::saveLog('5men refund_untreated_callback '.$data['data']['refund_round_id'], $this->provider_db_id, json_encode($data), $response);
					return $response;
				}
				...
				try {
					...
					Helper::saveLog('5men gameRefund success '.$data['data']['refund_round_id'], $this->provider_db_id, json_encode($data), 'success refund');
				  	return $response;
					...
					Helper::saveLog('5men ERROR catch'.$data["name"], $this->provider_db_id, json_encode($data), $e->getMessage());
					return $msg;
				}
			else:
				...
				Helper::saveLog('5men no bet found '.$data['data']['refund_round_id'], $this->provider_db_id, json_encode($data), $response);
				return $response;
			endif;
			...
			Helper::saveLog('5men duplicate error '.$data['data']['refund_round_id'], $this->provider_db_id, json_encode($data), $response);
			return $response;
		endif;
	}
	...
	public function getURL(Request $request){
        ...
        $requesttosend = [
          ...
          "version" => 1,
          "token" => $token,
          "game" => 5557, //game_code, game_id
          "settings" =>  [
            'user_id'=> $client_details->player_id,
            'language'=> $client_details->language ? $client_details->language : 'en',
          ],
        ...
        //dd($requesttosend);
        // $signature_checker = $this->getSignature($this->project_id, 2, $requesttosend, $this->api_key,'get_signature');
        ...
        // dd($requesttosend);
        // $signature =  '5fb07c5edc0441b36827aa138237bd90';
        // $requesttosend['signature'] = $signature;
	...
	public function gameInit($request){
		...
		if($client_details != null){
				...
				TGGHelper::saveLog('5men Balance Response '.$data['name'], $this->provider_db_id, json_encode($data), $data_response);
			...
			TGGHelper::saveLog('5men ERROR '.$data['name'], $this->provider_db_id,  json_encode($data), $data_response);
			return $data_response;
		}
	}
