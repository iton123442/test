...
class DigitainController extends Controller
{
	...
	public function betAllOrNone($json_data)
	{
		...
		foreach ($json_data['items'] as $key => $value) { // FOREACH CHECK
			...
			$game_details = DigitainHelper::findGameDetails('game_code', $this->provider_db_id, $value["gameId"]);
			if ($game_details == null) { // Game not found
				$items_array[] = [
					"info" => isset($value['info']) ? $value['info'] : '', // Info from RSG, MW Should Return it back!
	...
	public function win(Request $request){
		...
		foreach ($json_data['items'] as $key){
				...
				try {
		            ...
		            $income = $bet_transaction_status->bet_amount - $pay_amount;
		            if($pay_amount > 0){
		                $win_or_lost = 1;
		                $entry_id = 2;
		            }else{
		            ...
		            $provider_trans_id = $key['txId'];
		            $round_id = $datatrans->round_id;
		            $game_trans = $datatrans->game_trans_id;
			        ...
			        $client_response = ClientRequestHelper::fundTransfer_TG($client_details,$pay_amount,$game_details->game_code,$game_details->game_name,$game_trans,'credit',false,$action_payload);
	...
	public function winallOrNone($json_data){
		...
		foreach ($json_data['items'] as $key => $value ) { // FOREACH CHECK
				...
				# Compile all scanned data to variable json_data_ii
				array_push($json_data_ii, $value); 
			...
			foreach ($json_data_ii as $key) {
				...
				try {
					...
					$action_payload = [
			            ...
			            "provider" => [
			                "provider_request" => json_encode($json_data), #R
			                "provider_trans_id"=> $provider_trans_id, #R
			                "provider_round_id"=> $round_id, #R
			                "provider_name" => $game_details->provider_name,
			            ],
	...
	public function betwin(Request $request){
		...
		foreach ($json_data['items'] as $key){
				...
				if(isset($client_response->fundtransferresponse->status->code) 
			        ...
			        $client_response2 = ClientRequestHelper::fundTransfer_TG($client_details,$pay_amount,$game_details->game_code,$game_details->game_name,$game_trans,'credit',false,$action_payload);
	...
	public function betwinallOrNone($json_data){
		...
		if ($error_encounter != 0) { // ELSE PROCEED TO CLIENT TRANSFERING
			...
			foreach ($json_data_ii as $key) {
				...
				try {
					$client_response = ClientRequestHelper::fundTransfer($client_details, abs($key['betAmount']), $game_details->game_code, $game_details->game_name, $game_transextension, $game_trans, 'debit',$fund_extra_data);
				} catch (\Exception $e) {
					$items_array[] = array(
						"info" => $key['info'],
						"errorCode" => 999,
	...
	public function refund(Request $request){
		...
		foreach ($json_data['items'] as $key) { 
	    		...
	    		if($key['refundRound'] == true){
	    			...
	    			if($datatrans->game_transaction_type == 1){
						...
						if(isset($provider_request_payload->promoWinAmount)){
							...
							// if(isset($key['originalTxId']) && $key['originalTxId'] != null){
							...
							// 	}	
							// }else{
							// 	$is_bet_has_won = GameTransactionMDB::findGameExt($key['roundId'], 2,'round_id', $client_details);
							// 	if($is_bet_has_won != 'false'){
							// 		$items_array[] = [
							// 			"info" => $key['info'],
							// 			"errorCode" => 20,
	    			...
	    			// its a win round
						...
						// if(isset($key['roundId']) && $key['roundId'] != null){
						// 	$is_bet_has_won = GameTransactionMDB::findGameExt($key['roundId'], 2,'round_id', $client_details);
						// 	if($is_bet_has_won != 'false'){
						// 		$items_array[] = [
						// 			"info" => $key['info'],
			...
			if($datatrans != 'false'){ // TRANSACTION IS FOUND
					...
					try {
					$client_response = ClientRequestHelper::fundTransfer($client_details,abs($amount),$game_details->game_code,$game_details->game_name,$game_transextension,$datatrans->game_trans_id,$transactiontype,true,$fund_extra_data);
					} catch (\Exception $e) {
						$items_array[] = array(
							 "info" => $key['info'], 
							 "errorCode" => 999, 
		...
		foreach ($json_data['items'] as $key => $value) { // #1 FOREACH CHECK
				...
				if ($value['refundRound'] == true) {  // Use round id always
					...
					if(isset($provider_request_payload->promoWinAmount) || isset($provider_request_payload->chargeAmount)){
						...
						$db_provider_request_data = $this->findObjDataItem($datatrans->provider_request, $value['roundId'], 'playerId');
		 ...
		 //   continue;
			...
			if($isset_allbets_amount == 0){ # Calculate all total bets
				foreach ($json_data['items'] as $key2 => $key_amount) {
					...
					array_push($all_bets_amount, $key_amount['amendAmount']);
				}
				$isset_allbets_amount = 1;
			}
   ...
   //      	    ]; 
			...
			$fund_extra_data = [
	 		...
	 		try {
			 $client_response = ClientRequestHelper::fundTransfer($client_details,abs($amount),$game_details->game_code,$game_details->game_name,$game_transextension,$gametransaction_details->game_trans_id,$transaction_type,true,$fund_extra_data);
	...
	public function amendallOrNone($json_data){
		...
		foreach ($json_data['items'] as $key => $value) { // FOREACH CHECK
				...
				$json_data['client_details'] = $client_details;
				$value['client_details'] = $client_details;
				// $json_data['items'][$i - 1]['client_details'] = $client_details;
				...
				if (isset($value['winTxId'])) {
					// $checkLog = DigitainHelper::findGameExt($value['winTxId'], 123, 'transaction_id'); // Amend can amend Bet and Win Select All Possible?

					$checkLog = GameTransactionMDB::findGameExt($value['winTxId'], 2,'transaction_id', $client_details);
					if ($checkLog != 'false') {

						# If Not Included in the Operation Types
						if($this->getOperationType($value['winOperationType']) == 'false'){
							$items_array[] = [
								"info" => $value['info'], 
								"errorCode" => 18,  
								"metadata" => isset($value['metadata']) ? $value['metadata'] : '' 
						...
						# If not in win operation type
						if($this->getWinOpType($value['winOperationType']) == false){
							$items_array[] = [
								"info" => $value['info'], 
								"errorCode" => 18,  
								"metadata" => isset($value['metadata']) ? $value['metadata'] : '' 
						...
						$db_operation_type = 1;
						$debit_operation_type = 37; 
						if ($value['operationType'] != 37 && $value['operationType'] != 38) {
							$items_array[] = [
								"info" => $value['info'],
								"errorCode" => 19,
								"metadata" => isset($value['metadata']) ? $value['metadata'] : ''
						...
						if ($checkLog->amount < $value['amendAmount']) {
							$items_array[] = [
								"info" => $value['info'],
								"errorCode" => 18,
								"metadata" => isset($value['metadata']) ? $value['metadata'] : ''
						...
						if (isset($checkLog->general_details)) {
							$db_general_details = json_decode($checkLog->general_details);
							if (isset($db_general_details->provider->operationType)) {
								if ($value['winOperationType'] != $db_general_details->provider->operationType) {
									$items_array[] = [
										"info" => $value['info'],
										"errorCode" => 19,
										"metadata" => isset($value['metadata']) ? $value['metadata'] : ''
						...
						if ($checkLog->round_id != $value['roundId']) {
							$items_array[] = [
								"info" => $value['info'],
								"errorCode" => 7,
								"metadata" => isset($value['metadata']) ? $value['metadata'] : ''
				...
				// $is_refunded = DigitainHelper::findGameExt($value['txId'], 3, 'transaction_id');
				$is_refunded = GameTransactionMDB::findGameExt($value['txId'], 3,'transaction_id', $client_details);
				if ($is_refunded != 'false') {
					$items_array[] = [
						"info" => $value['info'], // Info from RSG, MW Should Return it back!
						"balance" => $client_details->balance,
		...
		if ($error_encounter != 0) { 
			...
			foreach ($json_data_ii as $key) {
				...
				// $is_refunded = isset($key['is_refunded']) ? $key['is_refunded'] : false;
  ...
  //   	    ]; 
		...
		try {
			$client_response = ClientRequestHelper::fundTransfer($client_details,abs($bet_amount),$game_details->game_code,$game_details->game_name,$game_transextension,$game_trans,'debit',$fund_extra_data);
		...
		if(isset($client_response->fundtransferresponse->status->code) 
	        ...
	        $client_response2 = ClientRequestHelper::fundTransfer_TG($client_details,abs($promo_amount),$game_details->game_code,$game_details->game_name,$game_trans,'credit',false,$action_payload);
	...
	public function makeCharge(Request $request){
		...
		# Custom Configs
		# Change Fund Amount to 0 when changeBalance is false
		if (isset($json_data["changeBalance"]) && $json_data["changeBalance"] == true){
			$fundAmount = abs($bet_amount);
		}else{	
			$fundAmount = 0;
		}
	...
	/**
	 * Key GameCode, value always desktop gamecode, check if provider send mobile game code then convert it to desktop to send to client
	 */
	...
	// # Custom Game Search With Static Game Code Filter
	// public  function findGameDetails($type, $provider_id, $game_code){
		
	// 		$gameId = $this->getGameId($game_code);
	// 		if($is_exist_gameid == false){
	// 			return null;
	// 		}
