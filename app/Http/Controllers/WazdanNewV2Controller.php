...
class WazdanNewV2Controller extends Controller
{
    ...
    public function getStake(Request $request){
        ...
        if($datadecoded["user"]["token"]){    
            ...
            $client_details = ProviderHelper::getClientDetailsCache('token', $datadecoded["user"]["token"]);
            if($client_details){
                $game_details = ProviderHelper::findGameDetailsCache('game_code', $this->prefix, $datadecoded["gameId"]);
        ...
        if($datadecoded["user"]["token"]){
            ...
            if($client_details){
                ...
                try{
                    ...
                    Helper::saveLog('responseTime(WazdanFailed)', 12, json_encode(["starting"=>$this->startTime,"response"=>microtime(true)]), microtime(true) - $this->startTime);
