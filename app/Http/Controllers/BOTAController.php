...
class BOTAController extends Controller{
        ...
        if(isset($client_details)){
            ...
            if($betDetails->result_count != 0){//check if bet history is not null
                ...
                if(count($checkRefundCount) == 2){//if canceled 2 times
                    ...
                    try{
                        $newProvTransID = $this->prefix.'_'.$betDetails->result_value[0]->c_idx;//last round's bet idx
    ...
    public function _cancelProcess($data,$client_details){
        ...
        if(isset($client_details)){
            ...
            $client_response = ClientRequestHelper::fundTransfer($client_details,round($data["price"],2),$gamedetails->game_code,$gamedetails->game_name,$gameExt->game_trans_id,$game_trans_ext_id,'credit',true, $fund_extra_data);
