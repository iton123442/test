...
class BNGController extends Controller
{
    ...
    private function betNotNullWinNotNull($data,$client_details,$game_details){
        ...
        $client_response = ClientRequestHelper::fundTransfer($client_details,$betAmount,$game_details->game_code,$game_details->game_name,$betGametransactionExtId,$gen_game_trans_id,"debit",false,$body_details);
    ...
    private function betNotNullWinNull($data,$client_details,$game_details){
        ...
        $client_response = ClientRequestHelper::fundTransfer($client_details,$betAmount,$game_details->game_code,$game_details->game_name,$betGametransactionExtId,$gen_game_trans_id,"debit",false,$fund_extra_data);
        if(isset($client_response->fundtransferresponse->status->code) 
        && $client_response->fundtransferresponse->status->code == "200"){
            $balance = number_format($client_response->fundtransferresponse->balance,2,'.', '');
            ProviderHelper::_insertOrUpdate($client_details->token_id, $client_response->fundtransferresponse->balance);
