 ...
 *	
 */
class DigitainNEWController extends Controller
{
		...
		foreach ($json_data['items'] as $key){
				...
				if(isset($client_response->fundtransferresponse->status->code) 
					...
					if($key['winAmount'] != 0){
						...
						GameTransactionMDB::updateGametransactionV2($updateGameTransaction, $datatrans->game_trans_id, $client_details); 
		 	  		}else{
		 	  			$updateGameTransaction = [
	...
	public function betwin(Request $request){
		...
		foreach ($json_data['items'] as $key){
				...
				if(isset($client_response->fundtransferresponse->status->code) 
					...
					ameTransactionMDB::createGameTransactionExtV2($gameTransactionEXTData,$game_transextension2,$client_details);
  ...
  //   	    ]; 
		...
		if(isset($client_response->fundtransferresponse->status->code) 
			...
			&& $client_response2->fundtransferresponse->status->code == "402"){
                ...
                ProviderHelper::queTransactionLogs($createGameTransactionLog2);
			}
	...
	public function makeCharge(Request $request){
		...
		// $game_trans = ProviderHelper::createGameTransaction($token_id, $game_details->game_id, $bet_amount, 0, $method, $win_or_lost, null, $payout_reason, $income, $provider_trans_id, $round_id);
		// $game_transextension = ProviderHelper::createGameTransExtV2($game_trans,$provider_trans_id , $provider_trans_id , $bet_amount, 1,$request->all());
        $game_trans = ProviderHelper::idGenerate($client_details->conenction_name,1);
        $game_transextension = ProviderHelper::idGenerate($client_details->connection_name,2);
		$gameTransactionData = array(
			"provider_trans_id" => $provider_trans_id,
			"token_id" => $token_id,
