...
class YGG002NewV2Controller extends Controller
{
    public $provider_id;
    public $org;
    ...
    public function wager(Request $request){
        ...
        try{
            ...
            GameTransactionMDB::createGametransactionV2($updateGameTransaction, $game_trans, $client_details);
    ...
    public function endwager(Request $request){
        ...
        if($checkTrans != 'false'){
            ...
            Helper::saveLog("YGG 002 else", $this->provider_id, json_encode($client_response), $response);
            $balance = round($client_response->fundtransferresponse->balance,2);
            ProviderHelper::_insertOrUpdateCache($client_details->token_id, $client_response->fundtransferresponse->balance);
            $response = array(
                "code" => 1006,
